...
class TrafficLightDetector(Node):
    ...
    def init_manual_mode(self):
        ...
        if self.show_camera_windows:
            ...
            cv2.setMouseCallback("Camera 1", self.mouse_callback)
    ...
    def yolo_image_callback(self, img_msg):
        ...
        best_detection = self.find_best_traffic_light_detection(results)
        ...
        if best_detection:
            ...
            self.get_logger().info(
                f"YOLO detected: '{best_detection.class_name}' ({model_color}) with confidence {best_detection.score:.2f}")
        ...
        self.gui_detection_status = (
            f"YOLO-Based ({detection_state}) | Model:{model_color} | EMA:{ema_color} | Thr:{self.yolo_confidence_threshold:.2f}"
        )
        self.update_gui()

        if rclpy.ok() and (self.show_camera_windows or self.show_control_windows or self.show_mask_windows):
            if self.show_camera_windows:
                self.draw_yolo_results(frame, results)
    ...
    def manual_image_callback(self, img_msg):
        ...
        self.process_and_update(frame)

    def process_and_update(self, frame):
        if self.show_camera_windows:
            self.threshold_pixels = cv2.getTrackbarPos("Threshold (Pixels)", "Camera 1")
        ...
        result = self.process_frame(frame, self.roi)

        if rclpy.ok() and (self.show_camera_windows or self.show_control_windows or self.show_mask_windows):
            if self.show_camera_windows:
                self.visualizing(result['result_img'], result['roi'], result['result'])
        ...
        final_color = detection['color']
        self.gui_state = final_color.capitalize()
        pixel_str = f"R:{detection['red_pixels']}, G:{detection['green_pixels']} / Thr:{self.threshold_pixels}"
        ...
        if final_color.lower() == 'green':
            self.trigger_green_mission()
        self.update_gui()

    def process_frame(self, frame, roi_tuple):
        ...
        if roi_tuple:
            ...
            if roi_img.size > 0:
                detection_result = self.detect_color_with_hsv_range(roi_img)
        ...
        if self.show_mask_windows:
            ...
            if mask_img is not None:
                ...
                cv2.imshow("Mask", np.zeros((100, 100, 3), dtype=np.uint8))

        return {'result': detection_result, 'result_img': result_img, 'roi': roi_tuple}
    ...
    def visualizing(self, frame, roi_tuple, detection_result):
        if roi_tuple:
            x, y, w, h = roi_tuple
            ...
            if self.roi_mode == 'manual' and self.edit_mode:
                ...
                color = (0, 255, 0) if detected_color == "Green" else (0, 0, 255)
            else:
                color = (255, 255, 0)
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
        if self.roi_mode == 'manual' and self.edit_mode:
            cv2.putText(frame, "ROI EDIT MODE", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            if self.roi_points and self.temp_roi_end_point:
                p1 = self.roi_points[0]
                p2 = self.temp_roi_end_point
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if not self.roi_points:
                ...
                if end_x > start_x and end_y > start_y:
                    self.roi = (start_x, start_y, end_x - start_x, end_y - start_y)
                    self.save_rois_to_file()
                self.roi_points = []
                self.temp_roi_end_point = None
        elif event == cv2.EVENT_MOUSEMOVE and self.roi_points:
            self.temp_roi_end_point = (x, y)
        ...
        if os.path.exists(self.ROI_FILE):
            try:
                ...
                if roi_data is not None:
                    ...
                    self.get_logger().info(f'Successfully loaded ROI from {self.ROI_FILE}')
    ...
    def save_rois_to_file(self):
        ...
        try:
            with open(self.ROI_FILE, 'w') as f:
                json.dump({'1': list(self.roi)}, f, indent=4)
            self.get_logger().info(f'Successfully saved ROI to {self.ROI_FILE}')
        except Exception as e:
