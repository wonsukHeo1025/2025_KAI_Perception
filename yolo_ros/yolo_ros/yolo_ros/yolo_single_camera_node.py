...
class YoloSingleCameraNode(Node):
    def __init__(self):
        super().__init__("yolo_single_camera_node")
        ...
        # 카메라 프로세서 초기화 (단일 카메라)
        self.camera_processor = YoloCameraProcessor(
            name='Camera 1',
            model=self.model,
        ...
        # 이미지 크기 설정 (선택적)
        self.camera_processor.imgsz_width = self.imgsz_width
        self.camera_processor.imgsz_height = self.imgsz_height
        
        self.get_logger().info("싱글 카메라 YOLO 노드 설정 완료")

def main(args=None):
    rclpy.init(args=args)
    node = YoloSingleCameraNode()
    
    # 멀티스레드 실행자 사용
    from rclpy.executors import MultiThreadedExecutor
    executor = MultiThreadedExecutor(num_threads=2)  # 메인 + 1개 카메라 처리
