...
담당: GPS·IMU 융합 기반 위치 추정 + YOLOv8 기반 콘 디텍션 + 카메라-라이다 센서 퓨전 및 객체 추적

RTK-GNSS, IMU, LiDAR, 듀얼 카메라를 함께 사용하는 자율주행차 Perception 워크스페이스입니다.  
저는 차량의 자기 위치 추정 정확도와 객체 인지 성능을 동시에 높이는 역할을 맡아, EKF 기반 로컬라이제이션, 커스텀 YOLO 콘 검출, 카메라-라이다 투영 및 객체 단위 센서 퓨전, Hungarian + UKF 기반 추적 파이프라인을 중심으로 구현했습니다.
