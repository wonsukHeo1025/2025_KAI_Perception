...
- **주요 메서드**:
  - `predict()`: 모든 트랙 예측
  - `update()`: 새 측정값으로 업데이트
  - `createNewTrack()`: 새 트랙 생성
  - `pruneTracks()`: 오래된 트랙 제거
- **수정 시**: 트랙 관리 정책, 연관 알고리즘 변경
...
- **주요 메서드**:
  - `predict()`: 다음 상태 예측
  - `update()`: 측정값으로 상태 업데이트
  - `predictSigmaPoints()`: IMU 보상된 시그마 포인트 계산
- **수정 시**: UKF 파라미터, 상태 모델, 색상 로직 변경

##### `imu_compensator.cpp/.hpp`
- **기능**: IMU 데이터 필터링 및 버퍼링
- **역할**:
  - EMA (지수이동평균) 필터링
  - Butterworth 저역통과 필터
  - 타임스탬프 기반 보간
- **주요 메서드**:
  - `addIMUData()`: 새 IMU 데이터 추가
  - `getCompensatedIMU()`: 필터링된 IMU 데이터 반환
...
2. 추적 처리
   └─> ukf_tracking_node.cpp
       └─> UKFTracker (tracking/ukf_tracker.cpp)
           ├─> Track (tracking/track.cpp) - 개별 트랙 UKF
           └─> IMUCompensator (tracking/imu_compensator.cpp) - IMU 필터링
...
| UKF 파라미터 조정 | `tracking/track.cpp` (alpha, beta, kappa) |
| IMU 필터 설정 | `tracking/imu_compensator.cpp` |
| 트랙 수명 정책 | `tracking/ukf_tracker.cpp` (max_missed_detections_) |
