...
def generate_launch_description():
    """
    Single Camera Perception Vision Pipeline Launch File
    
    Includes:
    - LiDAR driver (ouster_ros)
    - Camera drivers (usb_cam_1)
    - YOLO single camera node
    ...
    # YOLO single camera node
    yolo_single_camera_node = Node(
        package='yolo_ros',
        executable='yolo_single_camera_node',
        name='yolo_single_camera_node',
        output='screen',
        parameters=[],
    )
    ...
    # Single Camera-LiDAR fusion (calico)
    calico_single_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            PathJoinSubstitution([
                FindPackageShare('calico'),
                'launch',
                'calico_single.launch.py'
            ])
        ])
    )
    ...
    return LaunchDescription([
        lidar_driver_launch,
        usbcam1_node,
        yolo_single_camera_node,
        prism_launch,
        cone_detection_launch,
        calico_single_launch,
    ])
