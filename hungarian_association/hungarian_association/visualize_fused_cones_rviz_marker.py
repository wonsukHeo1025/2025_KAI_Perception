...
class FusedConeColorVisualizer(Node):
    ...
    def __init__(self, node_name='fused_cone_color_visualizer'):
        ...
        # --- Parameters ---
        self.declare_parameter('input_topic', '/fused_sorted_cones')
        ...
        self.declare_parameter('marker_fixed_z', 0.3) # Fixed height for visualization
    ...
    def _cone_data_callback(self, msg: ModifiedFloat32MultiArray):
        ...
        cones_data = [] # Store tuples of (x, y, class_name)

        # --- 1. Parse the incoming message ---
        num_cones = 0
        stride = 0
        if len(msg.layout.dim) == 2 and msg.layout.dim[1].size == 2:
            ...
            try:
                for i in range(num_cones):
                    ...
                    cones_data.append((x, y, class_name))

            except IndexError as e:
                 self.get_logger().error(f"Index Error while parsing cone data: {e}. Check message layout/data consistency.")
                 self._publish_delete_markers(msg.header.frame_id or "map", now)
                 return
            except Exception as e:
                self.get_logger().error(f"Unexpected error parsing cone data: {e}")
                self._publish_delete_markers(msg.header.frame_id or "map", now)
                return

        elif len(msg.data) > 0 and len(msg.layout.dim) == 0:
             # Fallback: Assume flat list of [x1, y1, x2, y2, ...] if layout is missing/empty
             self.get_logger().debug("Message layout missing or not 2D. Assuming flat [x,y] pairs.")
             if len(msg.data) % 2 == 0:
                 ...
                 self.get_logger().warn("Data has odd number of elements, cannot form [x,y] pairs.")
        ...
        for i, (x, y, class_name) in enumerate(cones_data):
            marker = Marker()
            ...
            # Position: Use x, y from data and fixed z from parameter
