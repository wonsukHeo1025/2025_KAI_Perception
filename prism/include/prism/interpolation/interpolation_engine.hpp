...
struct InterpolationConfig {
    ...
    // Azimuth matching
    // If > 0, replace per-beam azimuth sorting with uniform binning of this size
    int azimuth_bins = 0;                  // 0 = disabled (default)
    ...
    void loadFromYaml(const YAML::Node& node) {
        ...
        azimuth_bins = ConfigLoader::readNestedParam(node,
            "interpolation.azimuth_bins", azimuth_bins);
